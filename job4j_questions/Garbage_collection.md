...
Корневой набор ссылок - это статические и локальные переменные(+ не статических полей в классах). Соответственно объекты достижимые из корневых ссылок - живые,\
недостижимые - мертвые.  
В общем случае, граф объектов не должен меняться по мере обхода. Следовательно, приложение должно быть остановлено, пока идет сборка мусора.  
...
+ **Неверные реализации equals() и hashCode() (HashSet и HashMap)** 
