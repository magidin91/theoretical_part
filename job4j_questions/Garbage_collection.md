...
## Metaspace
Metaspace хранит системную информацию.
Разделение хипа, стека и метаспейса являются в значительной степени условными, поскольку они используют виртуальную память.
Но MetaSpace не используют кучу пространства и не учитываются как часть максимального размера кучи, т.е. -Xmx.  

В Metaspace виртуальная машина хранит **метаданные загруженных классов**,**всё статическое содержимое приложения**: 
...
Нативная память - это вся память, которую JVM может использовать для своих нужд. (сюда входит также и стек, и хип.)  
