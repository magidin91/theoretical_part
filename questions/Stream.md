...
```
Лямбда - это объект класса, который служит реализацией для функционального интерфейса и содержит реализацию метода.    
...
Например, мы можем использовать целевой тип как арумент метода, соответственно передавая разные лямбды, мы сможем испольовать разные  
...
его классе. В лямбда-выражении доступен также по ссылке this (явно илинеявно) вызывающий экземпляр объемлющего его класса. 
...
myOp(MyArrayOps::<Integer> countMatching - если передаем ссылку на метод в обобщенный целевой тип, то просто указывааем 
параметры типа класса перед методом.     
...
Обычно применяется в статисческих фабричных методах для создания объектов разных классов.   
...
данных можно управлять в try с ресурсами. Но, как правило, ьзакрывать приходится только те потоки данных, где источники данных требуют
...
Для перебора кроме оперции foreach, можн оиспользовать Iterator и Spliterator, работа с ним осуществляется также как в коллекциях.  
...
+ parallelStream() - поулчить параллельный стрим  
...
Это терминальная операция. В ней можно рассчитать любую функцию над элементами, каоторая принимает два элемента и возвращает элемент того же типа. 
...
## Peek  
Используется либо для дебагга, либо чтоыб зименить объекты в первональном стриме.    
  
Возвращает тот же стрим, но применяет функцию к каждому элементу стрима. **Обе ForEach(тераминальный) и Peek(конвеерный) - меняют объекты в источнике.**    
peek (Consumer action)  
Consumer -  void accept(T t);  

Этот метод часто испольуется  для дебагга, где вы хотите увидеть элементы, когда они проходят мимо определенной точки в трубопроводе, 
...
либо просто преобразовать его определенным образом.(например, поулчить квадрат каждого элемента)      
...
DouЬleStream mapToDouЬle(ToDouЬleFunction<? super Т> функция_ отображения)  

Перевод стрима дабл в инт:  
ArrayList<DouЬle> myList = new ArrayList<>();  
...
В качестве аргумента мы обычно передаем лямбду, которая маппит каждый элемент исходного стрима в стрим.      
...
маппим каждую строку в поток слов, а затем каждый поток сглаживается и мы получаем общий поток из всех элементов.   
...
Supplier mapFactory - HashMap::new, указываем конкретный тип мпы, которую хотим создать.                         
...
В groupingBy есть конструкторы с двумя аргументами:  
...
Если нужно в листе получить преобразованные объекты или их поле, или если хотим получить не лист, а сет итд
...
##	Что такое Optional?
Optional - это класс оболочка, призванный помочь разработчикам в обработке NullPointerException. Ряд операций сведения в Stream Api, такие как min, max, reduce, findFirst, findAny возвращают объект Optional<T>. Этот объект фактически обертывает результат операции. Здесь Т обозначает тип сохраняемого значения. Следует иметь в виду, что экземпляр класса Optional может содержать значение типа Т или быть пустым. 
