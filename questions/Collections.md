...
Классы можно параметризовать обобщенными типами, это обеспечивают безопасность типов и повторное использование кода. 
...
Данный код не скомпилируется из-за первой строки. На его примере  объясняется, почему он не должен компилироваться:
...
Если кастуем через конструкцию с (T) для помещения в параметризованный тип, то кастование не добавится.  
```java
...
+ Генерируются bridge-методы.(подробнее в следующем разделе)    
[подробнее](https://urvanov.ru/2016/04/28/java-8-%d0%be%d0%b1%d0%be%d0%b1%d1%89%d0%b5%d0%bd%d0%b8%d1%8f/#typeerasure)    
...
метод с List<Integer> более подходящий. И мы падаем с ошибкой. (и в методе и агрументе будут просто List list)   
...
2 Способ:    
Мы можем запомнить тип параметра - Class<T> typeParameterClass, передав его явно в конструктор и запомнив в переменной.  
...
На вершине иерархии в Java Collection Framework располагаются 2 интерфейса: Collection и Map. Коллекция представляет группу элементов. 
...
При вводе дублиру.щих значений в Set.of() выбросит IllegalArgumentException!.  
...
+ \+ быстрая вставка и удаление в середину списка (переписать next и prev и всё) если проитерировались итератором
...
Хештаблица не гарантирует тот же порядок элементов, что был при добавлении. (когда перебираем entryset)  
...
вам необходима такая возможность, поищите другую структуру данных, например TreeMap.  
...
Хеш-мапа обеспечивает очень быструю вставку, поиск и удаление, дает константное время если у ключа хорошая хеш-функция, 
...
К LinkedList  тримап не упадет, т.к. красно-черное дерево может делать балансировку.    
...
log2 п сравнений, чтобы найти правильное расположение нового элемента. Так, если древовидное множество уже содержит 1000 элементов, 
...
+ а в нем вызывается метод Node<K,V> newNode, который переопрделен в LinkedHashMap  
```java
...
```  
В afterNodeAccess(e); перезаписываеются связь.    
...
без увеличения затрат, связанных с уопрядочиванием в TreeMap.  
...
## HashSet  
Основан на HashMap, вместо value заглушка Object.  
В нем добавлен метод, который использует хешмапу. (в случае LinkedHashSet - LinkedHashMap) 
```java
private transient HashMap<E,Object> map; //в хешмапу также можн опоместить LinkedHashMap в случае LinkedHashSet
...
В HashSet есть пятый конструктор HashSet(int initialCapacity, float loadFactor, boolean dummy), когда мы вызываем этот конструктор, 
то создается **LinkedHashMap**:    
В конструкторе LinkedHashSet  вызывается родительский конструктор HashSet с этими параметрами и создается LinkedHashMap.   
```java
...
А при вставке вызывается метод  add из HashSet, в котором вызывается метод put из LinkedHashMap с заглушкой вместо value.    
...
**Т.е. для каждого объекта, т.к. хешкод будет разный**, даже если объекты имеют одинаковые поля. (Поэтому в собственных классах мы переопределяем методы)    

Обратите внимание, что идентификационная реализация hashCode() зависит от JVM.
...
В версиях 6 и 7 это случайно генерируемое число. В версиях 8 и 9 это число, полученное на основании состояния потока. (Состояние потока, объединённое с xorshift)
...
+ Для Long (int)(value ^ (value >>> 32)) ( смещаем верхние биты в диапазон int  и объединяем исключающим OR)
...
**Сложность алгоритма - это соотношение количества шагов алгоритма к количеству данных.**
...
1. Отбрасывание констант (все константные коэффициенты отбрасываются), т.к. мы оцениваем алгоритм на бесконечности, то  O(2n) и O(n) - одно и то же, как и O(n) + O (1) =  O(n)  
2. Неважная сложность: O(N^2 +N) = O(N^2),  одна сложность поглощает другую, если она значительно больше (значитльено больше = больше в 2 раза)  
...
Если выполнение циклов последовательн ои не зависит от выполнения друг друга, то сложность = O(A + B)  
