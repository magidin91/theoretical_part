...
Итератор реализует fast-fail поведение, т.е. если в овремя итерации листа, мы вставим элемент list.add(2), то мы получим 
ConcurrentModificationException. (это значит, что какой-то поток зименяет коллекцию не методом итератора)  
...
Хешкод, определенный в классе Object не изменится, если мы поменяем значения полей объекта, но если мы используем классы, 
где метод хешкод переопредлен (и встроенные, и кастомные), то при каждом вызове при изменении полей, будет менять хешкод.    
...
По сути сеты не имеют собственные реализации, а основаны на соответствующих мапах.  
...
LinkedHashМap расширяеи ноду HashМap и добавляет два - поля - предыдущий и следующий элемент. Также в классе опрелена головаи хвост, когда добавляем элемент,
записываем его в хвост, а в поле предыдущий элмент записываем хвост, у хвоста пишем следующим элементом этот.        
...
Но O(N^2 + B) = O(N^2 + B), выражение не может быть упрощено, т.к. мы ничего не знаем о B (меньше ли B в 2 раза), 
пока мы ничего не знаем о перменной мы не можем выбросить ее из формулы.  
...
## Коллекции общее: 
+ ArrayList - для произвольного доступа по индексу, т.е. чтение из  по индексу ArrayList - О(1)  
+ LinkedList - если будем вставлять и удалять из начала или вставлять в конец
(под вопросом, т.к. если в ArrayList выбран хороший размер, расширение массива будет происходить редко). 
Или если мы проитерировались до нужной позиции до вставка и удаление тоже.    
